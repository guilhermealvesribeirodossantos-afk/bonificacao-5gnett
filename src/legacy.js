...
function atualizarInterfaceGerencia() {
  ...
  if ($("usuarioNome")) {
    $("usuarioNome").textContent = gerenciaAutorizada
      ? "Gerência 5GNETT"
      : "Equipe 5GNETT";
  }

  if ($("usuarioPerfil")) {
    $("usuarioPerfil").textContent = gerenciaAutorizada
...
async function validarGerencia(token, userId) {
  ...
  return Array.isArray(dados) &&
    dados.some(item => String(item.user_id) === String(userId));
}
...
async function loginGerencia(email, senha) {
  ...
  const autorizado = await validarGerencia(
    dados.access_token,
    dados.user?.id
  );
...
function atualizarCamposCanal() {
  ...
  if (canal === "Ligação") {
    ...
    if (labelRelato) {
      labelRelato.textContent = "Relato da ligação *";
    }
    ...
    if (labelRelato) {
      labelRelato.textContent = "Relato do atendimento *";
    }
...
function pontosLiquidos(item) {
  return Number(item?.pontos || 0) +
    Number(item?.penalizacao || 0);
}
...
function rotuloPontuacao(item) {
  ...
  const erro = item?.tipo_erro
    ? ` • Erro ${String(item.tipo_erro).toLowerCase()}`
    : "";
...
function atualizarResumoAvaliacao() {
  const total =
    Number(avaliacaoAtual.pontos || 0) +
    Number(avaliacaoAtual.penalizacao || 0);

  if ($("avaliacaoPontuacaoFinal")) {
    $("avaliacaoPontuacaoFinal").textContent =
      avaliacaoAtual.classificacao
        ? `${total} pts`
        : "— pts";
  }
...
async function salvarAvaliacaoAtendimento() {
  ...
  const registro = {
    ...
    pontos:
      REGRAS_PONTUACAO[avaliacaoAtual.classificacao] || 0,
    penalizacao:
      REGRAS_PENALIZACAO[avaliacaoAtual.tipoErro] || 0,
    tipo_erro: avaliacaoAtual.tipoErro || null
  };

  try {
    const atualizado =
      await atualizarAtendimentoOnline(
        avaliacaoAtual.id,
        registro
      );
    ...
    alert(
      "Não foi possível salvar a avaliação no banco online."
    );
  } finally {
    if (botao) {
  ...
  tabela.innerHTML = listaPagina.map((item, i) => {
    ...
    return `
      <tr class="${pontuacaoJaAtribuida ? "atendimento-pontuado" : ""}">
        ...
        <td>
          <strong>${escaparHTML(item.atendente || "-")}</strong>
        </td>
...
async function excluirAtendimento(id) {
  ...
  if (!confirm(`Deseja excluir o atendimento de ${item.nome}?`)) {
    return;
  }
  ...
  try {
    ...
    alert(
      "Não foi possível excluir o atendimento do banco online."
    );
  }
}
...
function verRelato(id) {
  ...
  const tituloRelato =
    $("modalRelato")?.querySelector(".modal-header h2");
  ...
  $("textoRelato").textContent =
    item.relato || "Nenhum relato registrado.";
...
form.addEventListener("submit", async e => {
  ...
  try {
    ...
    alert(
      "Não foi possível salvar o atendimento no banco online. Verifique a conexão e as políticas do Supabase."
    );
  }
});
...
// ======================================================
// CORREÇÃO DOS BOTÕES DE NOVO ATENDIMENTO
// ======================================================

// Botão da página ATENDIMENTOS
...
// Botão da página INÍCIO
if ($("btnNovoAtendimentoInicio")) {
  $("btnNovoAtendimentoInicio").addEventListener(
    "click",
    () => abrirModal()
  );
}
...
if ($("menuNovoAtendimento")) {
  $("menuNovoAtendimento").addEventListener(
    "click",
    () => abrirModal()
  );
}
...
if ($("fecharModalRelato")) {
  $("fecharModalRelato").addEventListener(
    "click",
    fecharRelato
  );
}
...
document.querySelectorAll(".avaliacao-opcao").forEach(botao => {
  botao.addEventListener("click", () => {
    ...
    avaliacaoAtual.pontos =
      REGRAS_PONTUACAO[classificacao] || 0;
...
document.querySelectorAll(".avaliacao-penalizacao").forEach(botao => {
  botao.addEventListener("click", () => {
    avaliacaoAtual.tipoErro =
      botao.dataset.erro || "";

    avaliacaoAtual.penalizacao =
      Number(botao.dataset.penalizacao || 0);

    atualizarResumoAvaliacao();
  });
});

if ($("fecharModalAvaliacao")) {
  $("fecharModalAvaliacao").addEventListener(
    "click",
    fecharAvaliacao
  );
}
...
document.addEventListener("keydown", e => {
  if (e.key === "Escape") {
    ...
    if (modalRelato.classList.contains("ativo")) {
      fecharRelato();
    }

    if (
      $("modalLoginGerencia")?.classList.contains("ativo")
    ) {
      fecharLoginGerencia();
    }

    if (
      $("modalAvaliacao")?.classList.contains("ativo")
    ) {
      fecharAvaliacao();
    }

    if (
      $("modalPreviaFechamento")?.classList.contains("ativo")
    ) {
      fecharPreviaFechamento();
    }

    if (
      $("modalHistoricoFechamentos")?.classList.contains("ativo")
    ) {
      fecharHistoricoFechamentos();
    }
  }
});
...
function atualizarControlesFotoEquipe() {
  ...
  if (acoes) {
    acoes.hidden = !gerenciaAutorizada;
  }
}
...
async function enviarFotoEquipe(arquivo) {
  if (
    !gerenciaAutorizada ||
    !sessaoGerencia?.access_token
  ) {
    alert(
      "Entre na Gerência para alterar a foto."
    );
  ...
  const arquivoDestino =
    `${base}.${extensaoFotoEquipe(arquivo)}`;
  ...
  try {
    const resposta = await fetch(
      `${SUPABASE_URL}/storage/v1/object/${SUPABASE_FOTOS_BUCKET}/${arquivoDestino}`,
      {
        ...
        headers: {
          apikey: SUPABASE_KEY,
          Authorization:
            `Bearer ${sessaoGerencia.access_token}`,
          "Content-Type": arquivo.type,
          "x-upsert": "true"
        },
        body: arquivo
    ...
    if (foto) {
      foto.src =
        `${urlFotoEquipe(nome)}?v=${versao}`;
    }

    const card = document.querySelector(
      `[data-atendente-perfil="${nome}"] .equipe-avatar-v1`
    );

    if (card) {
      card.textContent = "";
      card.style.backgroundImage =
        `url("${urlFotoEquipe(nome)}?v=${versao}")`;
    }
...
function configurarUploadFotoEquipe() {
  ...
  input?.addEventListener(
    "change",
    () => enviarFotoEquipe(input.files?.[0])
  );
...
function atualizarPainelEquipe(
  nome = atendentePerfilSelecionado
) {
  ...
  const resolvidos = itens.filter(
    item => item.resolutividade === "Resolvido"
  ).length;
  ...
  const pontosBase = itens.reduce(
    (soma, item) =>
      soma + Number(item.pontos || 0),
    0
  );

  const penalizacaoTotal = itens.reduce(
    (soma, item) =>
      soma + Number(item.penalizacao || 0),
    0
  );

  const pontos =
    pontosBase + penalizacaoTotal;
  ...
  const simples =
    itens.filter(
      item => item.classificacao === "Simples"
    ).length;

  const normal =
    itens.filter(
      item => item.classificacao === "Normal"
    ).length;

  const complexo =
    itens.filter(
      item => item.classificacao === "Complexo"
    ).length;

  const excelente =
    itens.filter(
      item => item.classificacao === "Excelente"
    ).length;

  const errosLeves =
    itens.filter(
      item => item.tipo_erro === "Leve"
    ).length;

  const errosGraves =
    itens.filter(
      item => item.tipo_erro === "Grave"
    ).length;

  const totalPenalizacoes =
    errosLeves + errosGraves;
  ...
  if (fotoPerfil) {
    fotoPerfil.onload = () => {
      ...
      if (inicialPerfil) {
        inicialPerfil.style.display = "none";
      }
    };
    ...
    fotoPerfil.src =
      `${urlFotoEquipe(atendentePerfilSelecionado)}?v=1`;
  }

  document.querySelectorAll(
    "[data-atendente-perfil]"
  ).forEach(botao => {
    const nomeCard =
      botao.dataset.atendentePerfil || "";
  ...
  definirTexto(
    "perfilEquipeComplexo",
    complexo
  );

  definirTexto(
    "perfilEquipeExcelente",
    excelente
  );

  definirTexto(
    "perfilEquipeErroLeve",
    errosLeves
  );

  definirTexto(
    "perfilEquipeErroGrave",
    errosGraves
  );

  definirTexto(
    "perfilEquipeProgressoTexto",
    `${pontos} / 350 pts`
  );

  const barra =
    $("perfilEquipeProgressoBarra");
  ...
  const metaResumo =
    document.querySelector(
      "#paginaEquipe .equipe-meta-v1 small"
    );

  if (metaResumo) {
    if (pontos >= 350) {
      metaResumo.textContent =
        "Faixa superior atingida • +20%";
    } else if (pontos >= 250) {
      metaResumo.textContent =
        `${350 - pontos} pts para a faixa superior`;
    } else {
      metaResumo.textContent =
        `${250 - pontos} pts para atingir a meta`;
    }
  }
...
function atualizarRelatorios() {
  ...
  $("relNaoResolvidos").textContent =
    naoResolvidos;

  $("relTaxa").textContent =
    `${taxa}%`;

  $("relChatmix").textContent =
    chatmix;

  $("relLigacoes").textContent =
    ligacoes;
  ...
  $("tabelaRelatorioAtendentes").innerHTML =
...
function atualizarBonificacao() {
  ...
  if ($("bonifResolvidos")) {
    $("bonifResolvidos").textContent =
      resolvidos;
  }

  if ($("bonifTaxaEquipe")) {
    $("bonifTaxaEquipe").textContent =
      `${taxaEquipe}%`;
  }
  ...
  const ranking = nomes
    .map(nome => {
      ...
      const penalizacoes =
        itens.reduce(
          (s, a) =>
            s + Number(a.penalizacao || 0),
          0
        );

      const pontos =
        pontosBase + penalizacoes;
      ...
      ranking.map((item, indice) => {
        ...
        return `
          <article class="ranking-card">
            ...
            <span>
              ${item.qtd} atendimentos •
              média ${item.media
                .toFixed(2)
                .replace(".", ",")} pts •
              ${item.percentual}% resolutividade
            </span>

            <span>
              ${faixa}${
                item.penalizacoes
                  ? ` • penalizações ${item.penalizacoes} pts`
                  : ""
              }
            </span>
          </article>
  ...
  if (lider) {
    if ($("bonifLiderNome")) {
      $("bonifLiderNome").textContent =
        lider.nome;
    }

    if ($("bonifLiderTaxa")) {
      $("bonifLiderTaxa").textContent =
        `${lider.pontos} pts`;
    }

    if ($("bonifLiderResumo")) {
      $("bonifLiderResumo").textContent =
        `${lider.qtd} atendimentos • média ${lider.media
          .toFixed(2)
          .replace(".", ",")} pts • ${lider.percentual}% de resolutividade.`;
    }
...
const SUPABASE_FECHAMENTOS_TABLE =
  "fechamentos_mensais";

const ATENDENTES_BONIFICACAO = [
  "Guilherme",
  "Ronald",
  "Ivo",
  "Juarez"
...
function competenciaAtual() {
  ...
  return {
    ...
    competencia: agora.toLocaleDateString(
      "pt-BR",
      {
        month: "long",
        year: "numeric"
...
function competenciaEncerrada(ano, mes) {
  return new Date() >=
    dataLiberacaoFechamento(
      ano,
      mes
    );
}

function formatarDataLiberacaoFechamento(
  ano,
  mes
) {
  return dataLiberacaoFechamento(
    ano,
    mes
  ).toLocaleDateString("pt-BR");
}

function atendimentosDaCompetencia(
  ano,
  mes
) {
  const {
...
function montarFechamentoAtendente(
  nome,
  ano,
  mes,
  competencia,
  ...
  const pontosLiquidosTotal =
    pontosBrutos + penalizacoes;

  const simples =
    itens.filter(
      item =>
        item.classificacao === "Simples"
    ).length;
  ...
  return {
    ...
    faixa_superior: 350,
    meta_atingida:
      pontosLiquidosTotal >= 250,
    faixa_superior_atingida:
      pontosLiquidosTotal >= 350
  };
}

async function consultarFechamentosCompetencia(
  ano,
  mes
) {
  if (
...
async function atualizarStatusFechamentoMensal() {
  ...
  try {
    const registros =
      await consultarFechamentosCompetencia(
        ano,
        mes
      );

    const fechado =
      ATENDENTES_BONIFICACAO.every(
        nome =>
          registros.some(
            item =>
              item.atendente === nome
    ...
    console.error(
      "Erro ao consultar fechamento mensal:",
      erro
    );
...
function fecharPreviaFechamento() {
  $("modalPreviaFechamento")
    ?.classList.remove("ativo");

  previaFechamentoAtual = null;
}

function renderizarPreviaFechamento(
  resumo,
  competencia
) {
  const tabelaPrevia =
    $("previaFechamentoTabela");
  ...
  if (
    ...
    alert(
      "O modal de prévia não foi encontrado. Verifique se o App.jsx da etapa anterior está publicado."
    );
  ...
  if (
    ...
    const liberado =
      competenciaEncerrada(
        Number(referencia.ano),
        Number(referencia.mes)
      );
    ...
    confirmar.textContent =
      liberado
        ...
        : `Disponível em ${formatarDataLiberacaoFechamento(
            Number(referencia.ano),
            Number(referencia.mes)
          )}`;
    ...
    confirmar.title =
      liberado
        ? ""
        : `O fechamento definitivo será liberado em ${formatarDataLiberacaoFechamento(
            Number(referencia.ano),
            Number(referencia.mes)
          )}.`;
  }
    ...
    resumo.map(item => {
      ...
      const meta250 =
        item.meta_atingida
          ...
          : `${Math.max(
              0,
              250 - item.pontos_liquidos
            )} pts faltam`;

      const meta350 =
        item.faixa_superior_atingida
          ? "✓ Atingida"
          : `${Math.max(
              0,
              350 - item.pontos_liquidos
            )} pts faltam`;
      ...
      return `
        <tr>
          ...
          <td>
            ${Number(
              item.resolutividade || 0
            ).toFixed(0)}%
          </td>

          <td>
            ${item.pontos_brutos} pts
          </td>
...
async function fecharMesBonificacao() {
  ...
  if (!listaMes.length) {
    alert(
      `Não existem atendimentos em ${competencia} para fechar.`
    );
  ...
  try {
    ...
    if (existentes.length) {
      alert(
        `${competencia} já possui registros de fechamento. O sistema não criará um fechamento duplicado.`
      );
    ...
    console.error(
      "Erro ao validar fechamento existente:",
      erro
    );

    alert(
      "Não foi possível validar o histórico antes do fechamento."
    );
  ...
  const resumo =
    ATENDENTES_BONIFICACAO.map(
      nome =>
        montarFechamentoAtendente(
          nome,
          ano,
          mes,
          competencia,
...
async function confirmarFechamentoMensal() {
  ...
  try {
    ...
    alert(
      `${competencia} foi fechado com sucesso.`
    );

  } catch (erro) {
    console.error(
      "Erro ao fechar competência:",
      erro
    );

    alert(
      "Não foi possível realizar o fechamento mensal no banco online."
    );
...
function rotuloMetaHistorico(
  ...
  if (valor >= meta) {
    return `
      <strong class="historico-meta-atingida-v1">
        ✓ Atingida
      </strong>
    `;
  }
...
function renderizarHistoricoFechamentos(
  ...
  if (
    ...
    alert(
      "O modal de histórico não foi encontrado. Verifique se o App.jsx da etapa anterior está publicado."
    );
        ...
        const totalPontos =
          registrosOrdenados.reduce(
            (soma, item) =>
              soma +
              Number(
                item.pontos_liquidos || 0
              ),
            0
          );

        return `
          <article class="historico-competencia-v1 ${
            indice === 0
              ? "mais-recente"
              : ""
          }">
            ...
            <div class="historico-competencia-topo-v1">
              ...
              <div class="historico-competencia-indicadores-v1">
                ...
                <span class="historico-fechado-badge-v1">
                  FECHADO
                </span>
                  ...
                  ${registrosOrdenados.map(
                    item => `
                      <tr>
                        ...
                        <td>
                          <strong class="historico-pontos-liquidos-v1">
                            ${Number(
                              item.pontos_liquidos || 0
                            )} pts
                          </strong>
...
async function abrirHistoricoFechamentos() {
  ...
  lista.innerHTML = `
    <div class="historico-vazio-v1">
      ...
      <span>
        Consultando os fechamentos preservados no banco de dados.
      </span>
  ...
  try {
    ...
    lista.innerHTML = `
      <div class="historico-vazio-v1">
        ...
        <span>
          Verifique a conexão e as permissões da tabela de fechamentos.
        </span>
...
if ($("btnFecharMes")) {
  $("btnFecharMes").addEventListener(
    "click",
    fecharMesBonificacao
  );
}

if ($("fecharPreviaFechamento")) {
  $("fecharPreviaFechamento").addEventListener(
    "click",
    fecharPreviaFechamento
  );
}
...
if ($("cancelarPreviaFechamento")) {
  $("cancelarPreviaFechamento").addEventListener(
    "click",
    fecharPreviaFechamento
  );
}
...
if ($("confirmarFechamentoMensal")) {
  $("confirmarFechamentoMensal").addEventListener(
    "click",
    confirmarFechamentoMensal
  );
}
...
if ($("modalPreviaFechamento")) {
  $("modalPreviaFechamento").addEventListener(
    "click",
    e => {
      if (
        e.target ===
...
if ($("btnHistoricoFechamentos")) {
  $("btnHistoricoFechamentos").addEventListener(
    "click",
...
if ($("fecharHistoricoFechamentos")) {
  $("fecharHistoricoFechamentos").addEventListener(
    "click",
    fecharHistoricoFechamentos
  );
}
...
if ($("okHistoricoFechamentos")) {
  $("okHistoricoFechamentos").addEventListener(
    "click",
    fecharHistoricoFechamentos
  );
}
...
if ($("modalHistoricoFechamentos")) {
  $("modalHistoricoFechamentos").addEventListener(
    "click",
    e => {
      if (
        e.target ===
...
const menuBonificacao =
  $("menuBonificacao");

const btnNovoAtendimentoTopo =
  $("btnNovoAtendimentoTopo");
...
if (menuAtendimentos) {
  menuAtendimentos.addEventListener(
    "click",
...
if (menuEquipe) {
  menuEquipe.addEventListener(
    "click",
    () => mostrarPagina("equipe")
  );
}
...
if (menuBonificacao) {
  menuBonificacao.addEventListener(
    "click",
    () => mostrarPagina("bonificacao")
  );
}
...
if (btnNovoAtendimentoTopo) {
  btnNovoAtendimentoTopo.addEventListener(
    "click",
    () => abrirModal()
  );
}
...
if ($("btnFiltrarRelatorio")) {
  $("btnFiltrarRelatorio").addEventListener(
    "click",
    atualizarRelatorios
  );
}
...
if ($("btnLimparRelatorio")) {
  $("btnLimparRelatorio").addEventListener(
    "click",
    () => {
      $("relatorioDataInicial").value = "";
      $("relatorioDataFinal").value = "";
...
if ($("btnFiltrarBonificacao")) {
  $("btnFiltrarBonificacao").addEventListener(
    "click",
    atualizarBonificacao
  );
}
...
if ($("btnLimparBonificacao")) {
  $("btnLimparBonificacao").addEventListener(
    "click",
    () => {
      $("bonificacaoDataInicial").value = "";
      $("bonificacaoDataFinal").value = "";
...
if ($("btnLoginGerencia")) {
  $("btnLoginGerencia").addEventListener(
    "click",
    abrirLoginGerencia
  );
}

if ($("fecharLoginGerencia")) {
  $("fecharLoginGerencia").addEventListener(
    "click",
    fecharLoginGerencia
  );
}

if ($("cancelarLoginGerencia")) {
  $("cancelarLoginGerencia").addEventListener(
    "click",
    fecharLoginGerencia
  );
}
...
if ($("btnSairGerencia")) {
  $("btnSairGerencia").addEventListener(
    "click",
    sairGerencia
  );
}
...
if ($("modalLoginGerencia")) {
  $("modalLoginGerencia").addEventListener(
    "click",
    e => {
      if (
        e.target ===
...
if ($("formLoginGerencia")) {
  $("formLoginGerencia").addEventListener(
    "submit",
    async e => {
      e.preventDefault();

      const email =
        $("emailGerencia").value.trim();
      ...
      try {
        ...
        console.error(
          "Falha no login da Gerência:",
          falha
        );

        erro.textContent =
          falha.message ||
          "Não foi possível entrar na Gerência.";
...
configurarUploadFotoGerencia();
  ...
  try {
    ...
    console.log(
      "5GNETT: banco online conectado."
    );

  } catch (erro) {
    console.error(
      "5GNETT: falha ao carregar Supabase.",
      erro
    );
    ...
    alert(
      "Não foi possível conectar ao banco online. O sistema exibirá o backup local deste navegador."
    );
  }
}
